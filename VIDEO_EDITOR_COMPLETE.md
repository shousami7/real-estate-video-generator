...
### 1. 編集画面へのアクセス
- `/` から自動で `/video/editor` にリダイレクト
- 生成済み動画・外部動画どちらもそのままアップロード可能
...
│
└── templates/
    └── video_editor_ui.html           # ✅ 編集UI（/video/editor で使用）
...
```
1. /video/editor にアクセス
   ↓
2. 動画をアップロード (ドラッグ&ドロップ or ファイル選択)
   ↓
...
6. 4つのバリエーションから選択
   ↓
7. 「Apply to Frame」ボタンをクリック
   ↓
   POST /frames/apply → フレーム更新
   ↓
8. (他のフレームも編集可能 - ステップ4に戻る)
   ↓
9. 「Export」ボタンをクリック
   ↓
...
- ブラウザで `/video/editor` を開く（トップページから自動リダイレクト）
- 画面中央のアップロードゾーンに動画をドラッグ&ドロップ、または「Upload」ボタンをクリックしてファイルを選択
- アップロード完了後に自動で6フレームが抽出されます
...
2. 動画ダウンロードが自動的に開始
3. 別の動画を編集したい場合は右上の「Back」でアップロード画面に戻る
